...
export function mapDelhiveryStatus(status, statusCode = "") {
  ...
  if (/in transit|transit|bagged|connected|manifested/.test(value)) return "in_transit";
  if (/picked up|pickup complete|collected/.test(value)) return "picked_up";
  if (/pickup request/.test(value)) return "pickup_requested";
  if (/failed|damaged|lost/.test(value)) return "delivery_failed";
  if (/created|pending|not picked/.test(value)) return "shipment_created";
  return null;
}
...
export function canAdvanceFulfillment(current, next) {
  ...
  if (["ndr", "delivery_failed", "cancelled", "returned"].includes(to)) return true;
  if (["ndr", "delivery_failed"].includes(from)) return true;
